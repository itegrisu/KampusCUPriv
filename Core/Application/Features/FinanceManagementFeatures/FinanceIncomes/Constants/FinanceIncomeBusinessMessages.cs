namespace Application.Features.FinanceManagementFeatures.FinanceIncomes.Constants;

public static class FinanceIncomesBusinessMessages
{
    public const string SectionName = "FinanceIncome";

    public const string FinanceIncomeNotExists = "FinanceIncome Not Exists";
    public const string ProcessCompleted = "Process Completed";

    public const string SuccessCreatedFinanceIncomeMessage = "FinanceIncome Successfully Created";
    public const string SuccessDeletedFinanceIncomeMessage = "FinanceIncome Successfully Deleted";
    public const string SuccessUpdatedFinanceIncomeMessage = "FinanceIncome Successfully Updated";

    //public const string SuccessMovedRecord = "Selected Record Successfuly Moved";
    //public const string Ranked1stError = "The record you want to move up is ranked 1st";
    //public const string RankedLastError = "The record you want to move down is in the last row";

    public const string TechnicalError = "Technical Error";
    public const string NotFoundRecord = "No Record Found!";
    public const string IncorrectOperation = "Incorrect Operation";
    //public const string IdNumberAlreadyExists = "This Id Number is Already Registered in the System";

    public const string FinanceIncomeGroupNotExists = "Ýlgili Gelir Grubu Bulunamadý";
    public const string CurrencyNotExists = "Ýlgili Para Birimi Bulunamadý";
    public const string SucessUploadFile = "Dosya Baþarýyla Yüklendi";
}
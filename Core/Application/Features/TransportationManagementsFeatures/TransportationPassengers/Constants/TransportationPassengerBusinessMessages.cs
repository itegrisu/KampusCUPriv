namespace Application.Features.TransportationManagementFeatures.TransportationPassengers.Constants;

public static class TransportationPassengersBusinessMessages
{
    public const string SectionName = "TransportationPassenger";

    public const string TransportationPassengerNotExists = "TransportationPassenger Not Exists";
    public const string ProcessCompleted = "Process Completed";

    public const string SuccessCreatedTransportationPassengerMessage = "TransportationPassenger Successfully Created";
    public const string SuccessDeletedTransportationPassengerMessage = "TransportationPassenger Successfully Deleted";
    public const string SuccessUpdatedTransportationPassengerMessage = "TransportationPassenger Successfully Updated";
    
	//public const string SuccessMovedRecord = "Selected Record Successfuly Moved";
	//public const string Ranked1stError = "The record you want to move up is ranked 1st";
	//public const string RankedLastError = "The record you want to move down is in the last row";

    public const string TechnicalError = "Technical Error";
	public const string NotFoundRecord = "No Record Found!";
	public const string IncorrectOperation = "Incorrect Operation";
	//public const string IdNumberAlreadyExists = "This Id Number is Already Registered in the System";
	public const string SuccessReportedTransportationPassengerMessage = "Yolcu Basariyla Bildirildi";
    public const string SuccessReportedTransportationPassengerMultiMessage = "Yolcular Basariyla Bildirildi";
    public const string SuccessReportCancelTransportationPassengerMessage = "Yolcu Iptal Edildi";

}
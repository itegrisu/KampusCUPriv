...
namespace API.Controllers.OfferManagementControllers
{
         ...
         CreatedOfferFileResponse, DeletedOfferFileResponse, UpdatedOfferFileResponse, GetByGidOfferFileResponse>
    {
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> UploadOfferFile([FromBody] UploadOfferFileCommand uploadOfferFileCommand)
        {
            UploadOfferFileResponse response = await Mediator.Send(uploadOfferFileCommand);
            return Ok(response);
        }
    }
}

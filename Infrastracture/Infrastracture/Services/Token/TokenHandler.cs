...
namespace Infrastracture.Services.Token
{
    public class TokenHandler : ITokenHandler
    {
        ...
        public T.Token CreateAccessTokenForPartTime(PartTimeWorker partTimeWorker, int minute = 600)
        {
            var claims = new List<Claim>
            {
                  new Claim(ClaimTypes.NameIdentifier, partTimeWorker.Gid.ToString()),
                  //new Claim("http://schemas.yourdomain.com/claims/email",user.Email),
                  //new Claim("http://schemas.yourdomain.com/claims/isSystemAdmin", user.IsSystemAdmin.ToString()),
                  //new Claim("http://schemas.yourdomain.com/claims/isLoginStatus", user.IsLoginStatus.ToString())

                  new Claim(ClaimTypes.Email,partTimeWorker.UserName),
                  new Claim("isSystemAdmin","false"),
                  new Claim("isLoginStatus",partTimeWorker.IsLoginStatus.ToString())
            };
        ...
        public T.Token CreateAccessTokenForWorker(ReservationHotelStaff reservationHotelStaff, int minute = 600)
        {
            var claims = new List<Claim>
            {
                  new Claim(ClaimTypes.NameIdentifier, reservationHotelStaff.Gid.ToString()),
                  //new Claim("http://schemas.yourdomain.com/claims/email",user.Email),
                  //new Claim("http://schemas.yourdomain.com/claims/isSystemAdmin", user.IsSystemAdmin.ToString()),
                  //new Claim("http://schemas.yourdomain.com/claims/isLoginStatus", user.IsLoginStatus.ToString())

                  new Claim(ClaimTypes.Email,reservationHotelStaff.GsmNo),
                  new Claim("isSystemAdmin","false"),
                  new Claim("isLoginStatus",reservationHotelStaff.HotelStaffStatus.ToString())
            };

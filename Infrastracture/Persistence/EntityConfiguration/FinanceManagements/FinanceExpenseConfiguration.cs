...
namespace Persistence.EntityConfiguration.FinanceManagements
{
    public class FinanceExpenseConfiguration : IEntityTypeConfiguration<FinanceExpense>
    {
        public void Configure(EntityTypeBuilder<FinanceExpense> builder)
        {
            ...
            builder.HasOne(y => y.MoneySenderPersonnelFK).WithMany(u => u.SendedFinanceExpenses).HasForeignKey(y => y.GidMoneySenderPersonnelFK).IsRequired(false);
            builder.HasOne(y => y.MoneyReceivePersonnelFK).WithMany(u => u.ReceivedFinanceExpenses).HasForeignKey(y => y.GidMoneyReceivePersonnelFK).IsRequired(false);


            builder.HasOne(y => y.CurrencyFK).WithMany(u => u.FinanceExpenses).HasForeignKey(y => y.GidCurrencyFK);
            builder.HasOne(y => y.ApprovalReceiverFK).WithMany(u => u.ApprovedFinanceExpenses).HasForeignKey(y => y.GidApprovalReceiverFK).IsRequired(false);

...
namespace Persistence.EntityConfiguration.PersonnelManagements
{
    public class PersonnelAddressConfiguration : IEntityTypeConfiguration<PersonnelAddress>
    {
        public void Configure(EntityTypeBuilder<PersonnelAddress> builder)
        {
            ...
            builder.HasOne(y => y.UserFK).WithMany(u => u.PersonnelAddresses).HasForeignKey(y => y.GidPersonnelFK);
            builder.HasOne(y => y.CityFK).WithMany(u => u.PersonnelAddresses).HasForeignKey(y => y.GidCityFK);

            builder.Property(y => y.AddressTitle).IsRequired().HasColumnType("varchar").HasMaxLength(50);
            builder.Property(y => y.Address).IsRequired().HasColumnType("varchar").HasMaxLength(150);

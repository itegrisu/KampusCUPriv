...
namespace Persistence.EntityConfiguration.VehicleManagements
{
    public class VehicleAllConfiguration : IEntityTypeConfiguration<VehicleAll>
    {
        public void Configure(EntityTypeBuilder<VehicleAll> builder)
        {
            ...
            builder.HasMany(u => u.TransportationServices).WithOne(y => y.VehicleAllFK).HasForeignKey(y => y.GidVehicleFK); builder.HasMany(u => u.VehicleAccidents).WithOne(y => y.VehicleAllFK).HasForeignKey(y => y.GidVehicleFK);
        }
    }
}

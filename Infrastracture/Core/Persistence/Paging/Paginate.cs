namespace Core.Persistence.Paging
{
    public class Paginate<T> : IPaginate<T>
    {
        public Paginate(IEnumerable<T> source, int index, int size, int from)
        {
            ...
            if (source is IQueryable<T> queryable)
            {
                ...
                Items = queryable.Skip(((Index - 1) - From) * Size).Take(Size).ToList();
            }
            else
            {
                T[] enumerable = source as T[] ?? source.ToArray();
                Count = enumerable.Count();
                Items = enumerable.Skip(((Index - 1) - From) * Size).Take(Size).ToList();
            }
        }
    ...
    public class Paginate<TSource, TResult> : IPaginate<TResult>
    {
        ...
        )
        {
            ...
            {
                Count = queryable.Count();
                TSource[] items = queryable.Skip(((Index - 1) - From) * Size).Take(Size).ToArray();
            ...
            else
            {
                ...
                Count = enumerable.Count();
                TSource[] items = enumerable.Skip(((Index - 1) - From) * Size).Take(Size).ToArray();
                Items = new List<TResult>(converter(items));
            }
        }
